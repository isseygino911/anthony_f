...
export function ThemeSettings() {
  ...
  const [sectionStyles, setSectionStyles] = useState<
    Record<SectionKey, SectionStyle>
  >({
    hero: "gradient",
    featured: "flat",
  ...
  useEffect(() => {
    getTheme()
      .then((data) => {
        ...
        setCustomPrimary(
          data.custom_colors?.primary ?? data.resolvedColors.primary,
        );
        setCustomSecondary(
          data.custom_colors?.secondary ?? data.resolvedColors.secondary,
        );
      })
      .catch((err) =>
  ...
  function handleCustomColorChange(
    ...
    const next =
      which === "primary"
        ? { primary: value, secondary: customSecondary }
        : { primary: customPrimary, secondary: value };
  ...
  return (
    <div className="flex flex-col gap-6 max-w-3xl mx-auto">
      ...
      <p className="text-sm text-muted-foreground">
        Changes preview immediately below and across the app. Nothing is
        persisted until you click "Save theme" — navigating away discards
        unsaved edits.
      </p>
          ...
          {SECTION_KEYS.map(({ key, label }) => (
            <div key={key} className="space-y-1">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  handleSectionStyleChange(key, v as SectionStyle)
                }
              >
                <SelectTrigger>
                  <SelectValue />
      ...
      <div className="space-y-1">
        ...
        <Select
          value={defaultMode}
          onValueChange={(v) => setDefaultMode(v as "light" | "dark" | "auto")}
        >
